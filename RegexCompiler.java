...
public class RegexCompiler
{
	...
	//constructor
	public RegexCompiler(String phrase)
	{
		regexp = phrase;
	...
	public static void main(String [] args)
	{
		...
		try
		{
			RegexCompiler compiler = new RegexCompiler(args[0]);
			compiler.parse();
		}
		catch (Exception e)
		{
	...
	public int term() throws IllegalArgumentException
	{
		...
		if (pointer < regexp.length())
		{
			...
			{
				// Equivalent to A+ → AA*
				// Create a BR state looping back to t1, then proceed
				machine.addState("BR", t1, state + 1); // BR to t1 and next
				r = state;
				state++;
				pointer++;
			}
	...
	// 	// add the final terminating state (BR, -1, -1)
	// 	machine.addState("BR", -1, -1);
	...
	public void parse() throws IllegalArgumentException
	{
		// Set the placeholder branching state; will fix it after knowing initial
		machine.addState("BR", -1, -1); // state 0

		// Build the rest starting at state 1
		int initial = expression();

		// Set the real starting state
		machine.setState(0, 1, 1); // Always point to state 1

		// If any of the regexp is left unparsed, throw error
		if (pointer != regexp.length())
			error();

		// Add the final BR -1 -1 state
		machine.addState("BR", -1, -1);
